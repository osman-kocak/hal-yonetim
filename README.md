...
- 🧾 **Legal fatura onayı** — Üç giriş noktası, **tek bileşen**: giriş ekranındaki kutucuk → `/admin/fatura-onay` tam sayfa, admin sol menüsünde **Fatura Onayı**, ve admin ana sayfasındaki özet kutusu. Aynı liste kopyalanmadı; fark yalnızca sayfa başına kayıt (tam sayfada 25, dashboard'da 10). Yetki her üçünde `ADMIN` + `ACCOUNTING`: iki sekme — **Onay Bekleyenler** (varsayılan, eskiden yeniye — en uzun bekleyen en tepede) ve **Onaylılar**. Kesilen HER irsaliye kuyruğa düşer; muhasebeci satırdaki *Onayla* ile açılan alana resmi fatura numarasını yazar, `Exit.invoiceNo` dolunca irsaliye onaylı sayılır. Ayrı bir "durum" kolonu **yok** — sekmeler tam olarak bu alanın boş/dolu olmasıdır (iki kaynaklı gerçek er ya da geç ayrışır). Fatura no **serbest metin ama benzersiz**: format dayatılmaz (fatura serileri işletmeye göre değişir), ancak aynı numara iki irsaliyeye yazılamaz; çakışma kontrolü **büyük/küçük harf duyarsız** ve hata hangi irsaliyede kullanıldığını söyler. Düzeltme `ADMIN`+`ACCOUNTING`'e açık, **onayı geri alma yalnız `ADMIN`** (numarayı düzeltmek muhasebe işi, bağı koparmak değil). Her iki işlem de `AuditLog`'a eski→yeni olarak düşer. Rozetler dört yerde: widget, Takip & Geçmiş listesi, çıkış ekranındaki başarı kartı ve **basılan fişin başlığında** (yalnız numara girilmişse — fiş çoğu zaman faturadan önce basıldığı için sahada genelde boş çıkar, onay sonrası yeniden bastırınca görünür). **Baskı filtresi (varsayılan: Basılı)** — fatura teslim edilmiş fişe kesilir, basılmamış fiş kuyruğu şişirip yapılacak işi görünmez kılıyordu; ekran açılınca yalnız `printedAt` dolu fişler gelir. Filtre **kapatılabilir ve kaç kaydın gizlendiği yazılır**: `printedAt` 28 Ağustos 2026'da eklendi, öncesindeki fişlerde baskı izi yok (fiilen basılmış olsalar da) ve baskı işareti `window.print()` sonrası ateşle-unut gidiyor — iPad'de iptal edilen istek işareti hiç yazmıyor. Sert filtre o kayıtları muhasebeden tamamen gizler ve hiç faturalanamazlardı. Sol menü rozeti ekranın varsayılanıyla aynı sayıyı gösterir (basılı + onaysız), yoksa rozet 403 derken liste 94 satır açardı
- 🖨️ **İrsaliye basıldı rozeti** — `Exit.printedAt` / `printedBy` / `printCount`. Yazdırma tetiklendikten **sonra** ateşle-unut olarak işaretlenir (`window.print()` öncesine `await` konamaz, iOS izni düşürüyor) ve hata yutulur. İlk baskı anı **sabit**, yeniden baskılar yalnız sayacı artırır. Yazdır düğmesinin **her yolu** işaretler: `window.print()`, iOS'ta PDF sekmesi, masaüstünde doğrudan PDF indirme ve Takip ekranından yeniden baskı (ortak nokta: `store/printStore.js` → `markIrsaliyePrinted`). Basılmamış irsaliyede rozet **hiç çıkmaz** — "basılmadı" yazmaz, çünkü `printedAt` 28 Ağustos 2026'da eklendi ve öncesindeki fişlerin hepsi basılmıştı ama alanları boş.
  <br>**NE ÖLÇTÜĞÜ (2026-09-04 ölçümü):** "Yazdır'a basıldı", **"kâğıt çıktı" değil**. İki yönde de sapıyor — AirPrint paneli iptal edilse de "basıldı" yazılır (tarayıcı haber vermiyor, JS ile çözümü yok), ve düğmeye hiç basılmazsa fiş "basılmamış" kalır. Canlıda 28 Ağustos sonrası 252 fişin 94'ü işaretli; **kayıp değil, kullanım farkı**: işaretlenen fişlerin 92'si kesimden sonraki 60 sn içinde işaretlenmiş (mekanizma çalışıyor), işaretsizlerin `printCount`'u **0** yani istek hiç atılmamış. Kişi bazında: yılmaz 108 fişin 68'ini, murat 17'nin 9'unu, hamit 70'in **9'unu**, kadir 57'nin **8'ini** basmış. Yani düşük oran teknik değil operasyonel — `printedAt`'e dayanan her filtre bunu bilerek kullanmalı (bkz. fatura onayı baskı filtresi: varsayılan açık ama **kapatılabilir**)
...
- [ ] Fatura Onayı açılışta **Basılı** sekmesinde; listedeki her satırda "İrsaliye basıldı" rozeti var
- [ ] Sarı şeritte gizlenen kayıt sayısı yazıyor; şeride tıkla → **Tümü**'ne geçti, basılmamışlar da geldi
- [ ] Sol menüdeki rozet ile başlıktaki "N bekliyor" **aynı sayı** (Tümü'ne geçince değişmiyor)
